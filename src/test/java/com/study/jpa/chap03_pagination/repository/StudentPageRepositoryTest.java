...
@SpringBootTest
// JPA는 INSERT, UPDATE, DELETE시에 트렌잭션을 기준으로 동작하는 경우가 많음.
// 기능을 보장받기 위해서는 웬만하면 트렌잭션 기능을 함께 사용해야 합니다.
// 나중에 MVC 구조에서 Service 클래스에 아노테이현을 첨부하면 됩니다.
@Transactional
@Rollback(false)
class StudentPageRepositoryTest {
    ...
    void testBasicPagination() {
        ...
        Pageable pageInfo
                ...
                amount,
//                Sort.by("name").descending() // 정렬 기준 필드명! 컬럼명(x) descending을 붙이면 내림차순으로 정렬
                Sort.by(  // 기준을 두개 이상으로 정렬하는 방법
        ...
        //when
        Page<Student> students = pageRepository.findAll(pageInfo); // pageInfo를 안주면 페이징 안하고 전체 조회를 한다.
